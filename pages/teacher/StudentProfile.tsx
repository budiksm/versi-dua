...
const StudentProfile: React.FC = () => {
  ...
  // ... (handleOpenDetail and other helper functions remain the same)
  ...
  const activeHomeroomReferral = homeroomSessions.find(s => {
      // Find referrals TO_BK that haven't been resolved
      if (s.recommendation !== 'TO_BK') return false;
      
      // If it refers to specific incidents, check if they are still active
      const relatedIds = s.relatedRecordIds || [];
      if (relatedIds.length > 0) {
          const areIncidentsResolved = relatedIds.every(id => {
              const rec = records.find(r => r.id === id);
              return rec && rec.bkStatus === 'COMPLETED';
          });
          return !areIncidentsResolved; // If not all resolved, referral is active
      }
      
      // If purely preventive referral (no IDs), assume active unless very old or followed by newer BK
  ...
  return (
    <div className="space-y-8 pb-12 animate-fade-in relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* TAB INCIDENTS & HOMEROOM (Same as before, skipped for brevity in diff but required in final file - I will output logic for BK Tab below) */}
        {activeTab === 'INCIDENTS' && (
           <>
