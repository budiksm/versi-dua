...
import { 
  ...
  Link as LinkIcon, Loader2, Cloud, ChevronRight, Plus, Info
} from 'lucide-react';
...
const StudentProfile: React.FC = () => {
  ...
  const handleSaveIncident = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Laporan tersimpan aman di Cloud.');
      setIncidentNotes('');
      setSelectedIncident('');
      setRefreshKey(k => k + 1);
  ...
  const buildTimeline = (): StoryStep[] => {
    ...
    records.forEach(r => {
      ...
      story.push({
        ...
        actor: `Pelapor: ${r.recordedBy}`, description: r.notes || '-',
    ...
    sanctions.forEach(s => {
      story.push({
        ...
        actor: 'Sistem Kesiswaan', description: s.notes,
        statusLabel: s.redemptionStatus === 'COMPLETED' ? 'Selesai Penebusan' : 'Sanksi Aktif',
      ...
      {isSubmitting && (
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center text-white">
            <div className="bg-white p-8 rounded-2xl shadow-2xl flex flex-col items-center text-slate-800">
                ...
                <p className="text-sm text-slate-500 mt-2">Sedang mengamankan data di server Google.</p>
      ...
      <div className="grid grid-cols-1 gap-6">
        {activeTab === 'TIMELINE' && (
          <div className="space-y-4">
            {timeline.length === 0 ? (
              <div className="bg-white p-12 text-center rounded-2xl border border-dashed border-slate-300 text-slate-400"><Clock className="h-12 w-12 mx-auto mb-4 opacity-20" /><p className="font-medium">Belum ada riwayat aktivitas.</p></div>
            ) : (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
             ...
             <form onSubmit={handleSaveIncident} className="p-8 space-y-6">
                <div className="flex p-1 bg-slate-100 rounded-xl max-w-sm">
                   <button type="button" onClick={() => { setFormType(IncidentTypeCategory.VIOLATION); setSelectedCategory(''); setSelectedIncident(''); }} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${formType === IncidentTypeCategory.VIOLATION ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500'}`}>PELANGGARAN</button>
                   <button type="button" onClick={() => { setFormType(IncidentTypeCategory.ACHIEVEMENT); setSelectedCategory(''); setSelectedIncident(''); }} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${formType === IncidentTypeCategory.ACHIEVEMENT ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500'}`}>PENGHARGAAN</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Kategori</label><select required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={selectedCategory} onChange={e => { setSelectedCategory(e.target.value); setSelectedIncident(''); }}><option value="">-- Pilih Kategori --</option>{categories.filter(c => c.targetType === formType).map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}</select></div>
                   <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Jenis Kejadian</label><select required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={selectedIncident} onChange={e => setSelectedIncident(e.target.value)} disabled={!selectedCategory}><option value="">-- Pilih Kejadian --</option>{incidents.filter(i => i.categoryId === selectedCategory).map(inc => <option key={inc.id} value={inc.id}>{inc.name} ({inc.points} Pt)</option>)}</select></div>
                </div>
                <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Catatan Detail</label><textarea required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 h-32" placeholder="Detail kejadian..." value={incidentNotes} onChange={e => setIncidentNotes(e.target.value)} /></div>
                ...
                <button type="submit" disabled={isSubmitting || !selectedIncident} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all disabled:bg-slate-300"><Save className="h-5 w-5" /> Simpan Laporan</button>
             </form>
          </div>
        )}
        ...
        {activeTab === 'COACHING' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
             ...
             <form onSubmit={handleSaveCoaching} className="p-8 space-y-6">
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-sm text-blue-800 flex items-start gap-3"><Info className="h-5 w-5 shrink-0 mt-0.5" /><p>Catatan pembinaan untuk mendokumentasikan pertemuan tatap muka antara Guru dan Siswa.</p></div>
                <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Hasil Pembinaan / Komitmen Siswa</label><textarea required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 h-32" placeholder="Siswa berjanji..." value={coachingNotes} onChange={e => setCoachingNotes(e.target.value)} /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Rekomendasi Lanjut</label><select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={coachingRecommendation} onChange={e => setCoachingRecommendation(e.target.value as any)}><option value="NONE">Tidak Ada (Cukup Pembinaan)</option><option value="TO_BK">Rujuk ke Guru BK</option><option value="PARENT_CALL">Panggilan Orang Tua</option><option value="TO_KESISWAAN">Rujuk ke Kesiswaan</option></select></div>
                   <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Status Kasus</label><select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={coachingStatus} onChange={e => setCoachingStatus(e.target.value as any)}><option value="CLOSED">SELESAI (Masuk Riwayat)</option><option value="OPEN">DALAM PANTAUAN</option></select></div>
                </div>
                <button type="submit" disabled={isSubmitting || !coachingNotes} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all disabled:bg-slate-300"><CheckCircle2 className="h-5 w-5" /> Simpan Catatan Pembinaan</button>
             </form>
          </div>
        )}
        ...
        {activeTab === 'SANCTIONS' && (
           <div className="space-y-4">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 flex items-center gap-4 shadow-sm">
                 ...
                 <div><h3 className="font-bold text-lg text-slate-800">Status Kedisiplinan</h3><p className="text-sm text-slate-500">Monitoring sanksi formal yang diterbitkan oleh sistem.</p></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {sanctions.length === 0 ? (
                   <div className="md:col-span-2 bg-emerald-50 p-12 text-center rounded-2xl border border-dashed border-emerald-200 text-slate-500"><CheckCircle2 className="h-12 w-12 text-emerald-500 mx-auto mb-3" /><p className="font-bold">Siswa belum memiliki riwayat sanksi berat.</p></div>
                 ) : (
                   sanctions.map(s => (
                     <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                        <div className="flex justify-between items-start mb-3"><span className="px-3 py-1 bg-red-600 text-white text-xs font-black rounded-lg">{s.level}</span><span className="text-[10px] font-bold text-slate-400 uppercase">{new Date(s.assignedDate).toLocaleDateString()}</span></div>
                        <p className="text-sm font-bold text-slate-800 mb-1">Penyebab: <span className="font-normal text-slate-600">"{s.notes}"</span></p>
                        <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-100"><p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Tugas Penebusan:</p><p className="text-xs font-medium text-slate-700">{s.redemptionTask || 'Belum ada tugas.'}</p></div>
