...
const TeacherDashboard: React.FC = () => {
  ...
  const getDynamicThresholds = (rules: CoachingRule[]) => {
      const getRuleMin = (keyword: string, defaultVal: number) => {
          // Added safety check for rules and statusLabel to prevent crashes
    ...
    if (user) {
      ...
      if (user.roles.includes(Role.BK)) {
        ...
        stds.forEach(s => {
            ...
            // UPDATED: Use negative check (!== COMPLETED) to catch OPEN, undefined, or any future active statuses
  ...
  const getStudentsForStatModal = () => {
      ...
      if (selectedStatType.startsWith('BK_')) {
          ...
          students.forEach(s => {
              ...
              if (selectedStatType === 'BK_ACTIVE' && latestSession?.status !== 'COMPLETED') {
                  include = true;
                  note = 'Sesi Aktif';
  ...
  // UI BASELINE LOCKED – DO NOT MODIFY STRUCTURE WITHOUT APPROVAL
  return (
