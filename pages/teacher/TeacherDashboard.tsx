...
const TeacherDashboard: React.FC = () => {
  ...
  // BK Specific State (Refactored for New Logic)
  const [bkStats, setBkStats] = useState({
      activeCounseling: 0, // NEW + MONITORING + REFERRED + RETURNED
      routineMonitoring: 0, // MONITORING only
      completedHistory: 0, // COMPLETED only
      mandatoryCases: 0,
      referrals: 0,
      highRiskCount: 0, 
  });
  ...
  const refreshDashboard = () => {
    ...
    if (user) {
      ...
      if (user.roles.includes(Role.BK)) {
        ...
        let totalActive = 0; // NOT COMPLETED
        let totalMonitoring = 0; // MONITORING ONLY
        let totalCompleted = 0; // COMPLETED ONLY
        ...
        stds.forEach(s => {
            ...
            // Check student's active records
            const studentRecords = recs.filter(r => r.studentId === s.id);
            
            // 1. ACTIVE COUNSELING (Any non-completed/none status)
            ...
            // 3. COMPLETED HISTORY (Has at least one completed record)
  ...
  // ... (Other handlers like tasks, approval remain the same) ...
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* --- KESISWAAN DASHBOARD (UNCHANGED FOR NOW) --- */}
      {isKesiswaan && (
        // ... (Same Kesiswaan Dashboard)
            ...
            <div className="bg-slate-800 rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="bg-slate-700/50 rounded-xl border border-white/10 overflow-hidden"><div className="px-4 py-3 border-b border-white/10 flex justify-between items-center bg-white/5"><h3 className="font-bold text-sm flex items-center gap-2 text-orange-300"><ClipboardList className="h-4 w-4" /> Antrean Sanksi Otomatis</h3><span className="bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full">{pendingTaskSanctions.length}</span></div><div className="divide-y divide-white/5 max-h-60 overflow-y-auto">{pendingTaskSanctions.length === 0 ? (<div className="p-4 text-center text-slate-400 text-xs">Aman. Tidak ada sanksi baru yang butuh tugas.</div>) : (pendingTaskSanctions.map((item, idx) => (<div key={idx} className="p-3 hover:bg-white/5 transition-colors flex justify-between items-center"><div><div className="font-bold text-sm">{item.student.name} <span className="text-slate-400 font-normal">({item.className})</span></div><div className="text-xs text-slate-300 mt-0.5 flex items-center gap-2"><span className="text-red-400 font-bold">{item.level}</span> • Skor: {item.currentScore}</div></div><button onClick={() => handleOpenTaskModal(item)} className="px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold rounded shadow-sm">Beri Tugas</button></div>)))}</div></div>
                        <div className="bg-slate-700/50 rounded-xl border border-white/10 overflow-hidden"><div className="px-4 py-3 border-b border-white/10 flex justify-between items-center bg-white/5"><h3 className="font-bold text-sm flex items-center gap-2 text-blue-300"><HeartHandshake className="h-4 w-4" /> Rujukan Tindakan Lanjut (Dari BK)</h3><span className="bg-blue-600 text-white text-xs px-2 py-0.5 rounded-full">{bkHandledList.length}</span></div><div className="divide-y divide-white/5 max-h-60 overflow-y-auto">{bkHandledList.length === 0 ? (<div className="p-4 text-center text-slate-400 text-xs">Tidak ada rujukan eskalasi dari BK.</div>) : (bkHandledList.map((item, idx) => (<div key={idx} className="p-3 hover:bg-white/5 transition-colors flex justify-between items-center"><div><div className="font-bold text-sm">{item.student.name}</div><div className="text-xs text-slate-300 mt-0.5">{translateRecommendation(item.session.recommendation)}</div></div><Link to={`/teacher/student/${item.student.id}`} className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white text-xs font-bold rounded">Lihat</Link></div>)))}</div></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    {isBK && (
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div onClick={() => handleOpenStatModal('BK_ACTIVE')} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 cursor-pointer hover:bg-white/20 transition-all hover:scale-105">
                            ...
                            <p className="text-[10px] text-white/60 mt-1">Belum Selesai</p>
                        </div>

                        {/* PANTAUAN RUTIN */}
                        <div onClick={() => handleOpenStatModal('BK_ROUTINE')} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-indigo-300/30 cursor-pointer hover:bg-white/20 transition-all hover:scale-105">
                            <p className="text-indigo-200 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">Pantauan Rutin <ExternalLink className="h-3 w-3" /></p>
                            <p className="text-3xl font-bold mt-1">{bkStats.routineMonitoring}</p>
                            <p className="text-[10px] text-white/60 mt-1">Butuh Evaluasi</p>
                        </div>

                        {/* RISIKO TINGGI */}
                        <div onClick={() => handleOpenStatModal('BK_HIGH_RISK')} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-yellow-400/50 bg-yellow-900/20 cursor-pointer hover:bg-white/20 transition-all hover:scale-105">
                            ...
                            <p className="text-[10px] text-white/60 mt-1">Poin &gt; 80% Batas</p>
                        </div>

                        {/* RIWAYAT SELESAI */}
                        <div onClick={() => handleOpenStatModal('BK_COMPLETED')} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 cursor-pointer hover:bg-white/20 transition-all hover:scale-105">
                            <p className="text-emerald-200 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">Riwayat Selesai <History className="h-3 w-3" /></p>
                            <p className="text-3xl font-bold mt-1">{bkStats.completedHistory}</p>
                            <p className="text-[10px] text-white/60 mt-1">Total Ditangani</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="bg-white/10 rounded-xl border border-white/10 overflow-hidden">
                            <div className="px-4 py-3 border-b border-white/10 flex justify-between items-center bg-white/5">
                                <h3 className="font-bold text-sm flex items-center gap-2 text-red-200"><AlertCircle className="h-4 w-4" /> Siswa Wajib Konseling (Prioritas)</h3>
                            </div>
                            <div className="divide-y divide-white/10 max-h-60 overflow-y-auto">
                                {bkMandatoryList.length === 0 ? (
                                    <div className="p-4 text-center text-purple-200 text-xs">Tidak ada siswa yang mencapai ambang batas poin.</div>
                                ) : (
      ...
      {/* ... (Wali Kelas Dashboard section same as before) ... */}
      {myClasses.length > 0 && (
      ...
      {/* --- RECENT ACTIVITY & MODALS (UNCHANGED) --- */}
      {/* ... (Recent Activity, Stats Modal, Task Modal, etc.) ... */}
